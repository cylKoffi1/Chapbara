...
if(isset($_GET['id']) AND !empty($_GET['id'])){
  ...
  if($checkIfQuestionExists->rowCount() > 0){
      ...
      if($clientInfos['idAd'] == $_SESSION['idA']){
        $nom = $clientInfos['nomAd'];
        $prenom = $clientInfos['prenomAd'];
        $sexe =$clientInfos['sexeAd'];
        $date =$clientInfos['dateAd'];
        $tel = $clientInfos['telAd'];
        $dom = $clientInfos['domAd'];
        $email = $clientInfos['emailAd'];
        $metier =$clientInfos['metAd'];
        $specialite = $clientInfos['speAd'];
        $grade =$clientInfos['gradeAd'];
        $loginn = $clientInfos['userAd'];
        $mdp = $clientInfos['mdpAd'];
        $code = $clientInfos['codeAd'];
        $photo = $clientInfos['photoAd'];
...
if(!empty($_POST['sexeV']) AND !empty($_POST['nom']) AND !empty($_POST['prenom']) AND !empty($_POST['login']) AND !empty($_POST['date']) AND !empty($_POST['tel']) AND !empty($_POST['email'])){
  ...
  $domM =htmlspecialchars($_POST['dom']);

  if(empty($_POST['sexe'])){
  //Modifier les informations de la question qui possède l'id rentré en paramètre dans l'URL
  $editQuestionOnWebsite = $bdd->prepare("UPDATE `adh` SET `nomAd`=?,`prenomAd`=?,`dateAd`=?,`telAd`=?,`domAd`=?,`emailAd`=?,`userAd`=? WHERE idAd=?");
  $editQuestionOnWebsite->execute(array($nomM, $prenomM, $dateM, $telM,$domM, $emailM,$loginM, $idClient));
  }
  $edit = $bdd->prepare('UPDATE adh SET sexeAd = ? WHERE idAd = ?');
        ...
        <div class="col-md-8">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <hr class="horizontal dark">
              <form  method="post">
              <p class="text-uppercase text-sm">Infos Professionnelles</p>
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <?php 
                        ...
                        $sql->execute(array($_SESSION['id'],$infos['idDem']));
                        if($sql->rowCount()>0){
...
if(isset($_POST['valide'])){
  ...
  //Vérifier si les champs sont remplis
      ...
      if(($mdp1 == $clientInfos['mdpAd'])){
      //Modifier les informations de la question qui possède l'id rentrée en paramètre dans l'URL
      $editQuestionOnWebsite = $bdd->prepare('UPDATE adh SET mdpAd = ? WHERE idAd = ?');
      $editQuestionOnWebsite->execute(array($mdp2,$idA));
...
?>
        ...
        <?php
          if(isset($_POST['validePho'])){
              ...
              $upload_C = "../photoA/".$photo_C;
              move_uploaded_file($_FILES['image']['tmp_name'],$upload_C);
              
              //Modifier les informations de la question qui possède l'id rentrée en paramètre dans l'URL
              $editQuestionOnWebsite = $bdd->prepare("UPDATE `adh` SET `photoAd`='$photo_C'  WHERE `idAd`=?");
              $editQuestionOnWebsite->execute(array($idA));
