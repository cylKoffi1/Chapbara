        ...
        switch($err){
          ...
          case 'success':
            ...
            <div class="alert alert-danger">
              <strong>Succes:</strong> Facture envoyée
            </div>
            <?php
          break;
        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0">
              ...
              $req =$bdd->query('SELECT * FROM facture ORDER BY idFac DESC LIMIT 1');
              $dernier_client = $req->fetch();
              $req->closeCursor();
              $facture = "fac$dateAndTime 1";
        
              if(!empty($dernier_client)){
                $part = explode('_',$dernier_client['idFac']);
                $facture = "fac".'_'.$dateAndTime.(int)$part[0]+= 1;
              }
                        $addd = $bdd->prepare('SELECT *FROM adh WHERE idAd = ?');
                        $addd->execute(array($_SESSION['idA']));
                        if($addd->rowCount()>0){
                            while($adhe=$addd->fetch()){

   
                        $requtte =$bdd->prepare('SELECT * from client INNER JOIN demande ON client.idCb = demande.idCb  AND demande.idDem = ? ');
                        $requtte->execute(array($ids));
                        if($requtte->rowCount()>0){
                          while($affi = $requtte->fetch()){
                          ?>
              <h2 >Facture n° <input type="text" name="facture" value="<?= $facture?>" readonly>  </h2>
            </div>
    						...
    						<div class="row gutters">
    							<div class="col-lg-12 col-md-12 col-sm-12">
    								<div class="table-responsive">
                                      ...
                                      <center><button class="btn btn-danger btn-sm w-30 mb-3" name="validate"> ENVOYER</button></center>  
    								</div>
    							</div>
    						</div>
    						<!-- Row end -->
...
if(isset($_POST['validate'])){
  ...
  echo'<h1><center style="">Facture envoyée</center></h1>';
  }
